...
- 网络路由不会关闭 TLS 证书校验；会替换 HTTPS 证书的公共代理将被拒绝，避免平台 Cookie 暴露给代理服务商。
...
- 三种分析模式：本地模式不调用外部模型；轻量 LLM 模式只路由低置信、情感冲突、高互动、多议题和复杂长文本；全量 LLM 模式对去重文本并发分批复核。两种 LLM 模式失败时均回退本地模型。
...
- 风险议题：固定九类业务词表作为可解释基线，LLM 模式采用语义多标签，并对容易泛化的体验议题要求操作、射击、建造机制或 UI 原文证据；风险分由负向率 50%、高赞负向影响 35%、议题声量 15% 组成，只用于当前样本内排序。
...
LLM_LIGHTWEIGHT_MIN_ITEMS=800
LLM_LIGHTWEIGHT_MAX_RATIO=0.20
LLM_LIGHTWEIGHT_CONFIDENCE_THRESHOLD=0.70
```

轻量模式先用本地模型处理全量文本。去重样本不超过 800 条时全部送审；更多样本的送审预算取 800 条与唯一文本 20% 之较大值，但不会超过实际样本量。约三分之一预算按来源、内容类型与本地情感分层抽样，其余优先低置信、情感冲突、高互动、多议题和复杂长文本；聚合情感占比使用分层审计混淆矩阵做后分层校准，逐条证据仍保留其实际模型来源。全量模式发送全部去重文本。内容会先移除邮箱、手机号、QQ、URL 和 `@用户名`，不发送作者字段或 Cookie。报告分别显示送审率、校准样本量、送审成功率、模型一致率、token 用量与回退告警。

## 网络路由

“网络路由”页面支持服务器直连、可信手动代理和自动代理池。自动模式可选择 SCDN、ZDOpen 或智能组合，默认仅作用于 TapTap；节点按出口、目标站可达性、TLS 与延迟验证，TapTap 出现验证页时可在限定次数内换线重试。ZDOpen 凭据可通过页面本机配置或 `ZDOPEN_APP_ID`、`ZDOPEN_AKEY` 环境变量提供，响应同时兼容文档中的 `api` 与 `app_id` 参数形式。

严格 TLS 是默认策略。页面可显式勾选兼容模式以接入替换证书的代理，状态会持续显示风险标记，Chromium 仅对已选择的代理范围启用 HTTPS 错误兼容。该模式可能暴露登录内容，只应配合可信代理使用。ZDOpen 的代理提取接口本身仅提供 HTTP，akey 会直接发送给该供应商。
