...
“网络路由”页面支持服务器直连、可信手动代理和自动代理池。自动模式通过 `https://proxy.scdn.io/api/get_proxy.php` 按协议、国家代码和候选数量获取节点，逐批检测出口 IP 与延迟，再把通过严格 TLS 校验的节点应用到新建的 B站和 TapTap 浏览器会话。手动换线与自动换线会关闭当前平台页面并保留浏览器登录资料；采集或分析进行中禁止切换。

公共免费代理可能失效、监听流量或替换 HTTPS 证书。应用不会通过 `ignore_https_errors` 放行此类节点；需要稳定代理时应使用证书链正常的可信服务。
