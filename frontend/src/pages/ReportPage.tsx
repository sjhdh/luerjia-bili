...
function SourceSection({ section }: { section: ReportSection }) {
  ...
  return <section id={section.key} className={`source-report source-report-${section.key}`}>
    ...
    <div className="report-grid two-columns"><ChartPanel title="情感占比" subtitle={`${section.metrics.sample_count} 条有效样本`} option={sentimentOption(section.sentiment.items)} />{isTapTap && section.rating_distribution ? <ChartPanel title="星级分布" subtitle="星级作为权威情感标签" option={ratingOption(section.rating_distribution)} /> : <ChartPanel title="情感变化趋势" subtitle="按可用发布时间聚合" option={timelineOption(section.timeline)} />}</div>
