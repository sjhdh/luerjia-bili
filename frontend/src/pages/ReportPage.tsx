...
export default function ReportPage() {
  ...
  return (
    <article className="report-page">
      ...
      <header className="report-hero"><div className="hero-content"><div><p className="hero-kicker">双平台舆情报告</p><h1>《{report.keyword}》舆情分析</h1><p>{report.hero.subtitle}</p><div className="hero-chips"><span>B站 {report.metrics.selected_video_count} 个重点视频</span><span>{report.metrics.comment_count + report.metrics.danmaku_count} 条站内样本</span><span>TapTap {report.metrics.review_count} 条评价</span></div><small>生成于 {new Date(report.generated_at).toLocaleString("zh-CN")} · 登录用户可见页面采集</small></div>{report.hero.cover_url && <img className="hero-cover" src={report.hero.cover_url} alt={report.keyword} />}</div></header>
