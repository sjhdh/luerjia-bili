...
test("Bilibili login is rendered in an interactive page subwindow", async ({ page }, testInfo) => {
  ...
  await page.route("**/api/v1/platforms/bilibili/workspace", (route) => { workspaceOpens += 1; return route.fulfill({ json: platformState("bilibili", false) }); });
...
test("report separates source blocks and remains horizontally contained", async ({ page }, testInfo) => {
  ...
  for (const heading of ["跨平台情感占比", "B站官号", "B站相关视频", "TapTap 玩家评价", "AI 深度研判", "关键发现", "重点风险", "行动优先级", "分析质量", "合规与方法说明"]) {
...
test("report remains contained when resized from desktop to mobile", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "AI 深度研判" })).toBeVisible();
